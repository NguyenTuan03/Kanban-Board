...
import { deleteWorkspace } from "@/app/(dashboard)/dashboard/actions";
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setErrorMsg(res.message || "Đã xảy ra lỗi khi xóa không gian làm việc.");
      }
    } catch (err) {
      console.error(err);
      ...
      <div className="w-full max-w-md bg-slate-900 border border-white/10 rounded-2xl p-6 shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <div className="space-y-4">
          <p className="text-sm text-slate-300">
            Bạn có chắc chắn muốn xóa không gian làm việc <strong className="text-white">&quot;{workspaceName}&quot;</strong> không? Hành động này sẽ xóa vĩnh viễn tất cả các cột và các công việc liên quan. Không thể hoàn tác hành động này.
          </p>
          ...
          <div className="pt-2 flex gap-3">
            ...
            <button
              ...
              disabled={isSubmitting}
              className="flex-1 h-11 bg-rose-600 hover:bg-rose-500 disabled:bg-rose-600/50 text-white font-semibold text-sm rounded-xl cursor-pointer shadow-lg shadow-rose-600/20 transition-colors flex items-center justify-center gap-2"
            >
              {isSubmitting ? (
                <>
